    ...
    const waitForIframeLoad = () => {
      try {
        if (this.iframe!.contentWindow && this.iframe!.contentDocument) {
          console.log('VisualEditor: iframe loaded, attempting to inject script')

          // 检查是否已经注入过脚本
          if (this.iframe!.contentDocument.getElementById('visual-edit-script')) {
            console.log('VisualEditor: script already injected, enabling edit mode')
            this.sendMessageToIframe({
              type: 'TOGGLE_EDIT_MODE',
              editMode: true,
            })
        ...
        if (error instanceof DOMException && error.name === 'SecurityError') {
          console.error('VisualEditor: Same-origin policy violation. Make sure iframe content is served from the same origin.')
        }
      }
    }
